...
if __name__ == '__main__':
  ...
  #   # 0.01,
  # ]
  upsi_mains = np.linspace(0.001, 10, 100).round(2)
  upsi_refs = np.linspace(0.001, 10, 100).round(2)
  
  seeds = [seed for seed in range(10)]

  common_exp_args = [
    f"--optim=refmax_mixhebo",
    "--model_path=rf.pkl",
    "--n_suggestions=1",
    "--n_iters=100",
  ]
  ...
  for job_idx, (n_tasks, device,
    ...
    for task_idx in range(n_tasks):
      ...
      args.append(f"--output={_get_result_dir(launch_args.debug)}/refmax_mixhebo/{upsi_main[task_idx]}_{upsi_ref[task_idx]}/{seed[task_idx]}")
