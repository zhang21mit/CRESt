...
class MixHEBO(HEBO):
    ...
    def suggest(self, n_suggestions=1, fix_input = None):
        ...
        if self.X.shape[0] < self.rand_sample:
            ...
            iter  = max(1, self.X.shape[0] // n_suggestions)
            upsi_main = self.upsi_main
            upsi_ref = self.upsi_ref
            delta = 0.01
...
if __name__ == "__main__":
    ...
    parser.add_argument('--optim', type=str, default='refmax_mixhebo', 
        choices=[
            'hebo', 
            'mixhebo', 
            'max_mixhebo',
    ...
    parser.add_argument('--output_dir', type=str, default='./runs/real')
    parser.add_argument('--upsi_main', type=float, default=2)
    ...
    if args.start_iter > 1:
        opt = joblib.load(f'{args.output_dir}/opt{args.start_iter - 1}.pkl')
        opt.upsi_main = args.upsi_main
        opt.upsi_ref = args.upsi_ref
        print(f"Load from the previous checkpoint at: {args.output_dir}/opt{args.start_iter - 1}.pkl")
        print(f"Set upsi_main and upsi_ref as: {opt.upsi_main} and {opt.upsi_ref}")
    else:
        if args.optim == "hebo":
    ...
    if os.path.exists(f'{args.output_dir}/batch{args.start_iter}.xlsx'):
        print(f"Batch {args.start_iter} already exists.")
    else:
        # Load the observation from the last batch
        print(f"Load the observation from the last batch: {args.output_dir}/batch{args.start_iter - 1}.xlsx")
        priors = pd.read_excel(f'{args.output_dir}/batch{args.start_iter - 1}.xlsx')
        rec0, obj0 = priors[f.dim_names], priors[f.output_name]
        opt.observe(rec0, -obj0.to_numpy().reshape(-1, 1)) # Load to HEBO format

        # Suggest new params
        print(f"Propose {args.n_suggestions} new suggestions...")
        ...
        rec[f"sim_{f.output_name}"] = obj(rec) # Store in positive

        # Dump the predicted params
        rec.to_excel(f'{args.output_dir}/batch{args.start_iter}.xlsx', index=False)

        # Dump the normalized params
        rec[f.dim_names] = rec[f.dim_names].apply(lambda row: row / row.sum(), axis=1)
        rec.to_excel(f'{args.output_dir}/batch{args.start_iter}_norm.xlsx', index=False)
        
        # Save the checkpoint
        joblib.dump(opt, f'{args.output_dir}/opt{args.start_iter}.pkl')
        print(f"Save the results and checkpoint at: {args.output_dir}/batch{args.start_iter}.xlsx and {args.output_dir}/opt{args.start_iter}.pkl")

        print(f"Best: {min(opt.y.min(), -rec[f'sim_{f.output_name}'].max())}")